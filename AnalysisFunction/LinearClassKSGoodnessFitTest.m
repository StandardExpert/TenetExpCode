function sig = LinearClassKSGoodnessFitTest(array1,array2,ifDrawPlot,saveOutcomeFolder)
%反比例 类K-S 拟合优度检验
...
%---------------生成基本量
MonteCarloMethodRepeatTime = 100;
alpha = 0.05;
...
fun1 = fit(xList1,yList1,functionAnalytic, ...
    "StartPoint", [0, 0 ,0]); %根据自定义拟合函数f来拟合数据x，y
...
deltaD = abs(fun1(tempXX) - fun2(tempXX));
basicD = max(deltaD);
...
for ii = 1:MonteCarloMethodRepeatTime
    ...
    "StartPoint", [1000, 0 ,100]);
    %----------获得D
    deltaD = abs(tempFun1(tempXX) - tempFun2(tempXX));
...
sig = find(basicD > totalDArray,1,'first') / length(totalDArray);
%% ------------------------------------------------------------------------
%                                end.画图
%--------------------------------------------------------------------------
if ifDrawPlot
    ...
    %----------Save
    pngFileName = [saveOutcomeFolder '\LinearClassKSGoodnessFitTest' ];
    ...
    %----------Save
    pngFileName = [saveOutcomeFolder '\LinearClassKSGoodnessFitTest_Sig'];
    print(pngFileName,'-dpng','-r600');
    close;
end

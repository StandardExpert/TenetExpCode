...
itemNumberX = 3;
itemNumberY = 4;
itemNumber = itemNumberX * itemNumberY;
sequenceLength = 6;
% participantNumber = '201811061199';
StimulusGeneratorSpatial(participantNumber,trainNumber,testNumber,itemNumber,sequenceLength);
load([participantNumber 'StimulusSpatial.mat'],'stimulusCell');
...
cd([rootDir '\SpatialStim']);
% 读取
darkLEDMatrix = imread('darkLED.png');
darkLEDTexture = Screen('MakeTexture', windowPtr, darkLEDMatrix);
for ii = 1:itemNumber
    LEDMatrix = imread([num2str(ii) '.png']);
    LEDTexture(ii) = Screen('MakeTexture', windowPtr, LEDMatrix);
end
...
for trialNumber = 1:trainNumber
    ...
    for stimSequenceIndex = 1:sequenceLength
        %-----刺激
        Screen('DrawTexture', windowPtr, LEDTexture(stimSequence(stimSequenceIndex)), [], []); % Draw texture into the backbuffer
        vbl = Screen('Flip', windowPtr);
        WaitSecs(stimulusShowTime);
        %-----空屏
        Screen('DrawTexture', windowPtr, darkLEDTexture, [], []); % Draw texture into the backbuffer
        vbl = Screen('Flip', windowPtr);
    ...
    %-----呈现未被点亮的LED
    Screen('DrawTexture', windowPtr, darkLEDTexture, [], []);
    vbl = Screen('Flip', windowPtr);
    ShowCursor(0); 
    ...
    for mouseClicksNumber = 1:sequenceLength
        ...
        fullLinkValue = MousePositionJudgment(mouseX,mouseY,itemNumberX,itemNumberY,screenSize);
        Screen('DrawTexture', windowPtr, LEDTexture(fullLinkValue), [], []); % Draw texture into the backbuffer
        vbl = Screen('Flip', windowPtr);
        %等这一次松手了才继续。
        while button(1)
...
%--------------------------------------------------------------------------
t=toc;
allOutcomeName = [rootDir '\Data\' answer{1} ' TenetSpacial ' testdate '.mat'];
...
%%调用的子函数
function fullLinkValue = MousePositionJudgment(mouseX,mouseY,itemNumberX,itemNumberY,screenSize)
%判断X的bin和Y的bin，最终得到坐标，转成线性。
mouseX
mouseY
Xbin = ceil( (mouseX-560) / 200 )
Ybin = ceil( (mouseY-240) / 200 )
if Xbin<=0
    Xbin = 1;
end
if Xbin>4
